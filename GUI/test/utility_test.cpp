...
TEST_P(UtilityDeathTestFixture,
       GivenInvalidStartAndEndRange_WhenGeneratingNumber_ThenAssertIsThrown) {
    const auto&[range_start, range_end] = GetParam();

    EXPECT_DEATH(Utility::GenerateRandomNumberInRange(range_start, range_end), "")
                        << fmt::format("Failed at start value {} and end value {}", range_start, range_end);
}

INSTANTIATE_TEST_SUITE_P(UtilityParametrizedDeathTest, UtilityDeathTestFixture,
